...
//
// API "Common service APIs": Public Resource Client
//
// Command:
// $ goagen
...
// DownloadSchema downloads /files with the given filename and writes it to the file dest.
// It returns the number of bytes downloaded in case of success.
func (c *Client) DownloadSchema(ctx context.Context, filename, dest string) (int64, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "http"
	}
	p := path.Join("/schema/", filename)
